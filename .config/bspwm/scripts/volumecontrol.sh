...
function notify_vol
{
    ...
    bar=$(seq -s "." $(((($vol + 10) / 20) + 1)) | sed 's/[0-9]//g')
    dunstify "t2" -a "$vol$bar" "$nsink" -i $ico -r 91190 -t 2000
}
...
function notify_mute
{
    ...
    if [ "$mute" == "true" ] ; then
        dunstify "t2" -a "muted" "$nsink" -i ${icodir}/muted-${dvce}.svg -r 91190 -t 2000
    else
        dunstify "t2" -a "unmuted" "$nsink" -i ${icodir}/unmuted-${dvce}.svg -r 91190 -t 2000
    fi
}
